...
//  - WebSocket 구독 핸들 ref 관리 및 언마운트 시 메모리 누수 방지
//  - any 타입 제거, 예시 API 응답 전용 타입 정의
// ============================================================
...
/** 미국 중부 표준시(CST, UTC-6)와 한국 표준시(KST, UTC+9) 시차 보정값(시간 단위) */
// ============================================================
// 타입 정의
// ============================================================
...
export interface JoinedGroup {
     ...
     * 백엔드 GroupPost 엔티티의 author.email이 그대로 직렬화되어 내려옵니다.
     * 없을 경우 authorName(이메일 백업)으로 보조 판정합니다. */
    authorEmail?: string
}
 ...
 * ChatMessageEntity 또는 ChatMessage DTO의 직렬화 결과입니다.
 */
...
const formatToLocalTime = (dateInput: string | number[] | number | null | undefined): string => {
    ...
    try {
        ...
        // 예외 케이스 1: 이미 "HH:mm" 형태인 경우 그대로 반환
        if (/^\d{2}:\d{2}$/.test(dateStr)) {
            debugLog('formatToLocalTime', '이미 정제된 HH:mm 형식 등 그대로 반환', dateStr)
            return dateStr
        }

        let year = 2026, month = 5, day = 26, hours = 0, minutes = 0

        // 예외 케이스 2: 배열 형태 [year, month, day, hour, minute]
        if (Array.isArray(dateInput)) {
            ...
            // 예외 케이스 2-b: 쉼표로 이어진 문자열 "2026,5,26,1,14"
            const parts = dateStr.split(',').map(Number)
            year    = parts[0] ?? 2026
            month   = parts[1] ?? 5
            day     = parts[2] ?? 26
            ...
            // 예외 케이스 3/4: ISO 문자열 또는 시간 문자열
            // 브라우저의 자동 UTC 해석을 막기 위해 정규식으로 숫자만 추출합니다.
            const matches = dateStr.match(/\d+/g)

            if (!matches) {
                debugLog('formatToLocalTime', '숫자 추출 실패 원본 반환', dateStr)
                return dateStr
            }
            ...
            if (matches.length >= 5) {
                ...
                // "01:14:00" 처럼 시간 데이터만 들어온 경우
                hours   = parseInt(matches[0], 10)
                minutes = parseInt(matches[1], 10)
                debugLog('formatToLocalTime', '시간 전용 파싱', { hours, minutes })

            } else {
                debugLog('formatToLocalTime', '파싱 불가 원본 반환', dateStr)
                return dateStr
            }
        }

        // 핵심 보정: 미국 대 한국 시차 +KST_OFFSET_HOURS 적용
 ...
 * useChatRoom
 *
 * 채팅방 웹소켓 연결, 메시지 관리 및 작성자 권한 판정을 담당하는 커스텀 훅입니다.
 *
 * @returns
...
export function useChatRoom() {
    ...
    /** 현재 구독 중인 채널 핸들. 언마운트 시 명시적 unsubscribe 필요 */
    ...
    /**
     * useMemo로 memoize:
     * auth.currentUser는 Firebase 내부 상태이며 React 렌더 사이클 밖에서 관리되므로
     * 최초 실행 시점에 값을 캡처합니다.
     ...
     *
     * [판정 우선순위]
     * 1. group.authorEmail이 있으면 currentUserEmail과 직접 비교 (정확)
     * 2. group.authorName이 있으면 currentUserEmail과 비교 (닉네임 사용 시 false 가능)
     *
     ...
     * fetchJoinedGroups() 내부에서 g.author?.email로 추출하고 있습니다.
     */
    const isMyGroup = useCallback((group: JoinedGroup | null | undefined): boolean => {
        ...
        // 2순위: authorName (닉네임이 없을 때 이메일을 백업으로 내려주는 백엔드 로직 이용)
        //        닉네임이 설정된 유저는 authorName이 닉네임이므로 불일치할 수 있습니다.
        if (group.authorName) {
    ...
    }, []) // formatToLocalTime은 순수 함수(모듈 스코프에 정의)이므로 의존성 배열에 추가 불필요

    // ============================================================
    // 자원 정리 헬퍼 및 구독 해제, 연결 해제
    // ============================================================
    ...
    const connect = useCallback(async (gid: number, groupTitle: string) => {
        ...
        client.connect(
            ...
            () => {
                ...
                // 실시간 메시지 구독 및 구독 핸들을 ref에 저장해 나중에 해제 가능하게 합니다.
     ...
     * 빈 문자열, 연결 없음, 및 미인증 상태에서는 전송하지 않습니다.
     */
    ...
    // ============================================================
    // 언마운트 시 안전 정리 (메모리 누수 방지)
    // ============================================================
    useEffect(() => {
        return () => {
            debugLog('useEffect cleanup', '컴포넌트 언마운트 시 웹소켓 정리')
            cleanupConnection()
        }
    }, [cleanupConnection])
    ...
    return {
        ...
        /** 웹소켓 연결 완료 여부 */
        isConnected,
        /** 현재 로그인한 유저의 이메일 (권한 판정 UI에서 사용 가능) */
        currentUserEmail,
        /** 특정 그룹 채팅방에 연결 */
        connect,
        /** 메시지 전송 */
...
/**
 * 현재 로그인한 유저가 참여 중인 그룹 목록을 가져옵니다.
 *
