...
// 글 수정 함수 (간이 방식)
  const handleEditNavigation = (id: number) => {
    navigate(`/groups/edit/${id}`); // 수정 페이지 주소로 이동
  };

  useEffect(() => {
    fetchGroups();        // 전체 그룹 목록 가져오기
    fetchJoinedGroups();  // 📌 내가 참여 중인 그룹 ID들 가져오기 (추가)
  }, []);
  ...
  const fetchJoinedGroups = () => {
    // 실제 구현 시에는 로그인된 유저의 정보를 기반으로 호출해야 합니다.
    axios.get('http://localhost:8080/api/groups/joined-ids')
        .then(res => {
          setJoinedGroupIds(res.data); // DB에 저장된 참여 목록으로 상태 업데이트
        })
        .catch(err => console.error("참여 목록 로딩 실패", err));
  };
        ...
        <div style={{ maxWidth: '1000px', margin: '40px auto', padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {filteredGroups.map((group: any) => {
              // 📌 1. 여기서 각 그룹의 상태를 계산합니다.
              const isClosed = group.status === 'CLOSED' || group.currentCapacity >= group.maxCapacity;
              const statusLabel = isClosed ? '모집완료' : '모집중';
              const statusColor = isClosed ? '#94a3b8' : '#10b981';
              const statusBg = isClosed ? '#f1f5f9' : '#ecfdf5';
                  ...
                  <div key={group.id} style={{ backgroundColor: 'white', borderRadius: '20px', border: '1px solid #f1f5f9', overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.02)' }}>
                    ...
                    <div onClick={() => setSelectedId(selectedId === group.id ? null : group.id)} style={{ padding: '24px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        <div style={{ fontSize: '14px', color: '#64748b' }}>호스트: {group.authorName || '익명'}</div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                        {/* 📌 동적으로 변하는 상태 라벨 */}
                        <span style={{ backgroundColor: statusBg, color: statusColor, padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold' }}>
                                            {statusLabel}
                                        </span>
                    ...
                    {selectedId === group.id && (
                        <div style={{ padding: '0 30px 30px 30px', borderTop: '1px solid #f8fafc', backgroundColor: '#fff' }}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '40px' }}>
                            <button
                                disabled={isClosed || joinedGroupIds.includes(group.id)}
                                onClick={(e) => { e.stopPropagation(); handleJoinRequest(group); }}
                                style={{
                                  backgroundColor: isClosed ? '#cbd5e1' : (joinedGroupIds.includes(group.id) ? '#e2e8f0' : '#ff8a3d'),
                                  color: isClosed ? '#94a3b8' : (joinedGroupIds.includes(group.id) ? '#94a3b8' : 'white'),
                                  border: 'none', padding: '15px 60px', borderRadius: '12px',
                                  fontWeight: 'bold', cursor: isClosed ? 'default' : 'pointer', fontSize: '16px'
                                }}
                            >
                              {isClosed ? '모집이 마감되었습니다' : (joinedGroupIds.includes(group.id) ? '참여 중' : '참여 신청')}
                            </button>
                            <button
                                onClick={() => handleEditNavigation(group.id)}
